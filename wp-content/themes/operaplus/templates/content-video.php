			...
			<div class="box-image">
				<?php
					if (has_post_thumbnail ()) {
						...
						$url = $thumb ['0'];
						echo '<a href="'. get_permalink() .'" title="'. get_the_title() .'" class="box-link"><img src="' . $url . '" alt="'. get_the_title() .'"><span class="box-format-icon box-format-icon--video"></span></a>';
					} else {
						$content = get_the_content();
						$imageUrl = get_image_url($content);	
						if(!empty($imageUrl)) {
							echo '<a href="'. get_permalink() .'" title="'. get_the_title() .'" class="box-link"><img src="' . $imageUrl . '" alt="">';
						} else {
